...
export const VocabularyTooltip: React.FC<VocabularyTooltipProps> = ({
  ...
  return (
    <div ref={containerRef} className={`relative inline ${className}`}>
        ...
        return (
          <span
            ...
            className={`inline-block cursor-pointer rounded px-0.5 transition-all ${
              isSelected ? 'bg-sky-500 text-white font-bold shadow-xs' : 'hover:bg-sky-100 hover:text-sky-900 border-b border-dotted border-sky-400'
            }`}
      ...
      {activeWord && (
        <div
          style={{ position: 'fixed', left: `${tooltipPos?.x || 20}px`, top: `${tooltipPos?.y || 100}px`, zIndex: 9999 }}
        ...
        >
          <div className="flex items-center justify-between border-b border-slate-800 pb-2">
            <div className="flex items-center space-x-2 min-w-0">
              ...
              {lookupData?.cefrLevel && <span className="px-1.5 py-0.5 bg-sky-500/20 text-sky-300 text-[10px] font-bold rounded border border-sky-400/30">{lookupData.cefrLevel}</span>}
            </div>
            <div className="flex items-center space-x-1 shrink-0">
              <button type="button" onClick={() => speakText(activeWord)} className="p-1 hover:bg-slate-800 text-sky-400 rounded-lg" title="Sesli dinle"><Volume2 className="w-4 h-4" /></button>
              <button type="button" onClick={closeTooltip} className="p-1 hover:bg-slate-800 text-slate-400 hover:text-white rounded-lg" title="Kapat"><X className="w-4 h-4" /></button>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              <div className="rounded-xl border border-amber-700/60 bg-amber-950/50 p-3 text-xs text-amber-100">
                <div className="flex items-start gap-2"><AlertTriangle className="w-4 h-4 shrink-0 mt-0.5" /><div><div className="font-bold">Doğrulanmış anlam alınamadı</div><p className="mt-1 text-[11px] text-amber-200">{lookupError}</p><p className="mt-1 text-[10px] text-amber-300">Bu nedenle kelime tekrar destesine eklenemez.</p></div></div>
              </div>
              <button type="button" onClick={() => void lookupWord(activeWord)} className="w-full py-2 rounded-xl font-bold text-xs flex items-center justify-center gap-1.5 bg-slate-700 hover:bg-slate-600 text-white"><RefreshCw className="w-3.5 h-3.5" /><span>Tekrar Dene</span></button>
            </div>
          ) : lookupData ? (
            <div className="space-y-2 text-xs">
              ...
              {onAddLearningItem && (
                <button type="button" onClick={handleAdd} disabled={added} className={`w-full py-2 rounded-xl font-bold text-xs flex items-center justify-center space-x-1.5 transition-all ${added ? 'bg-emerald-600 text-white' : 'bg-sky-600 hover:bg-sky-500 text-white shadow-md'}`}>
                  {added ? <><Check className="w-3.5 h-3.5" /><span>Uygulama Tekrar Destesine Eklendi</span></> : <><PlusCircle className="w-3.5 h-3.5" /><span>Kelimeyi Tekrar Destesine Ekle</span></>}
                </button>
              )}
            </div>
          ) : null}
