...
function promptFor(action: string, payload: JsonRecord): string {
  ...
  switch (action) {
    ...
    case '/api/generate-error-quiz': return `${guard}\nGenerate a three-question multiple-choice quiz at CEFR ${asString(payload.cefrLevel) || 'A1'} difficulty for a Turkish-speaking adult learning ${asString(payload.language) || 'the target language'}.\nIf payload.topicFocus is present (grammarTopic/grammarPattern), the quiz teaches that specific grammar topic at the given CEFR level — treat it as a curriculum lesson check, not an error report; questions must relate to that topic and the explanation must teach the rule briefly in Turkish.\nOtherwise, the errorTopics and focusCategory in the reference data are the learner's ACTUAL recurring mistakes — every question must relate to them.\n${data}\nReturn exactly {"quizTitle":"title","questions":[{"id":"q1","fossilizedErrorId":null,"errorCategory":"category","turkishPrompt":"prompt","options":["A","B","C","D"],"correctOptionIndex":0,"explanationTr":"explanation"}]}`;
