...
function promptFor(action: string, payload: JsonRecord): string {
  ...
  switch (action) {
    ...
    case '/api/lookup-word': return `${guard}\nGive a contextual dictionary explanation IN THE TARGET LANGUAGE ITSELF: a simple, learner-friendly monolingual definition of the word in its context. Do NOT translate the definition to Turkish. Also provide a short Turkish gloss only as a secondary aid. The example sentence must stay in the target language.\n${data}\nReturn exactly {"meaningTarget":"simple definition in the target language","translationTr":"short Turkish gloss (secondary aid)","grammaticalRole":"role","cefrLevel":"A1|A2|B1|B2|C1|C2","exampleSentence":"example sentence in the target language","exampleTranslationTr":"Turkish translation of the example"}`;
